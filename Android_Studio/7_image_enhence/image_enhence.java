...
public class YourService extends KiboRpcService {
    ...
    private void cropAndSaveRegion(Mat image, org.opencv.core.Point[] cropPoints2D) {
        try {
            ...
            // Set CLAHE parameters (adjust these as needed)
            clahe.setClipLimit(2.0);  // Controls contrast enhancement (1.0-4.0 recommended)
            clahe.setTilesGridSize(new Size(8, 8));  // 8x8 grid works well for 320x320
            
            // Apply CLAHE to the YOLO image
            ...
            Log.i(TAG, String.format("CLAHE enhanced image (320x320) - Min: %.2f, Max: %.2f", 
                claheMinMaxResult.minVal, claheMinMaxResult.maxVal));
            
            // Save the CLAHE-enhanced image
            api.saveMatImage(claheImage, "yolo_clahe_320x320.png");
            Log.i(TAG, "CLAHE-enhanced image saved as 320x320");

            // ========================================================================
            // BINARIZE THE CLAHE-ENHANCED IMAGE
            // ========================================================================
            
            // Option 1: Fixed threshold binarization
            Mat binarizedFixed = new Mat();
            double threshold = 127.0;
            double maxValue = 255.0;
            Imgproc.threshold(claheImage, binarizedFixed, threshold, maxValue, Imgproc.THRESH_BINARY);
            
            // Print min/max values of fixed threshold binarized image
            Core.MinMaxLocResult binaryFixedResult = Core.minMaxLoc(binarizedFixed);
            Log.i(TAG, String.format("Binary fixed (127) - Min: %.2f, Max: %.2f", 
                binaryFixedResult.minVal, binaryFixedResult.maxVal));
            
            // Save the fixed threshold binarized image
            api.saveMatImage(binarizedFixed, "yolo_binary_fixed_320x320.png");
            Log.i(TAG, "Fixed threshold binary image saved as 320x320");

            // Option 2: Otsu's automatic threshold (often works better with CLAHE)
            ...
            // Save the Otsu binarized image
            api.saveMatImage(binarizedOtsu, "04_yolo_binary_otsu_320x320.png");
            Log.i(TAG, String.format("Otsu binary image saved (threshold: %.1f)", otsuThreshold));


            // // Binarize the image using threshold
            // Mat binarizedImage = new Mat();
            // double threshold = 127.0; // You can adjust this threshold value
            // double maxValue = 255.0;
            // Imgproc.threshold(yoloImage, binarizedImage, threshold, maxValue, Imgproc.THRESH_BINARY);

            // // Save the YOLO-style image
            // api.saveMatImage(binarizedImage, "yolo_cropped_region_320x320.png");
            // Log.i(TAG, "Cropped region saved as YOLO-style 320x320 image");
            ...
            claheImage.release();
            binarizedFixed.release();
            binarizedOtsu.release();
